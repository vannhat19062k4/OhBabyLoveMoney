...
import { cashSignFor, debtActions, expenseCategories, expenseOptions, incomeOptions, reportingBucketFor, type DebtAction, type TransactionGroup } from '@/lib/finance-taxonomy';
import { getSupabaseBrowserClient } from '@/lib/supabase/client';

type View = 'overview' | 'pending' | 'transactions' | 'accounts' | 'cards' | 'budgets' | 'debts';
type Transaction = { id: string; icon: string; name: string; category: string; time: string; occurredAt?: string; emailId?: string; amount: number; group: TransactionGroup; reporting: 'income' | 'expense' | 'none'; accountId?: string; accountName?: string; destinationAccountId?: string; destinationAccountName?: string; debtAction?: DebtAction; debtId?: string; person?: string; due?: string; cardDelta?: number; systemType?: 'credit-card-payment' };
...
type AppData = { transactions: Transaction[]; budgets: Budget[]; debts: Debt[]; accounts: FinancialAccount[]; pending: PendingImport[]; processedEmailIds: string[]; cardBalance: number; cardLimit: number; cardStatementDay: number; cardDueDay: number };
...
const emptyData = (): AppData => ({ transactions: [], budgets: [], debts: [], accounts: [], pending: [], processedEmailIds: [], cardBalance: 0, cardLimit: 0, cardStatementDay: 12, cardDueDay: 27 });
...
export default function HomePage() {
  ...
  const [cardStatementDay, setCardStatementDay] = useState(12);
  ...
  const [transactionPreset, setTransactionPreset] = useState<Transaction | null>(null);
  ...
  useEffect(() => {
    ...
    const applyData = (data: Partial<AppData> | null) => {
      ...
      setCardStatementDay(typeof safe.cardStatementDay === 'number' ? safe.cardStatementDay : 12);
      setCardDueDay(typeof safe.cardDueDay === 'number' ? safe.cardDueDay : 27);
    };
    ...
    const load = async () => {
      ...
      try {
        ...
        authenticated = true;
        try {
          const saved = localStorage.getItem(`ohbabylovemoney-data-v3:${sessionUser.id}`);
          if (saved) localData = JSON.parse(saved);
        } catch { /* bản sao offline của đúng tài khoản */ }
        setAccountId(sessionUser.id);
        setAccountEmail(sessionUser.email || '');
        const { data: cloud, error: cloudError } = await supabase.from('user_app_state').select('payload').eq('user_id', sessionUser.id).maybeSingle();
  ...
  useEffect(() => {
    if (!ready) return;
    const data = { transactions, budgets, debts, accounts, pending, processedEmailIds, cardBalance, cardLimit, cardStatementDay, cardDueDay };
    if (accountId) {
      try { localStorage.setItem(`ohbabylovemoney-data-v3:${accountId}`, JSON.stringify(data)); }
      catch { /* Cloud vẫn là bản lưu chính nếu bộ nhớ offline của trình duyệt đã đầy. */ }
    }
  ...
  }, [ready, authState, accountId, accountEmail, transactions, budgets, debts, accounts, pending, processedEmailIds, cardBalance, cardLimit, cardStatementDay, cardDueDay]);
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`user-state:${accountId}`).on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'user_app_state', filter: `user_id=eq.${accountId}` }, (change) => {
      ...
      setCardStatementDay(payload.cardStatementDay ?? 12);
      setCardDueDay(payload.cardDueDay ?? 27);
      setSyncStatus('synced');
    }).subscribe();
  ...
  const applyTransactionEffects = (transaction: Transaction, direction: 1 | -1) => {
    if (typeof transaction.cardDelta === 'number') setCardBalance((balance) => Math.max(0, balance + transaction.cardDelta! * direction));
    else if (transaction.accountId === CREDIT_CARD_ID) setCardBalance((balance) => Math.max(0, balance - transaction.amount * direction));
  ...
  return (
    <main className="min-h-dvh bg-[#f2f2f7] text-[#1c1c1e]">
      {notice && <output className="fixed left-1/2 top-4 z-[80] w-max max-w-[90vw] -translate-x-1/2 rounded-full bg-[#17231f] px-4 py-2.5 text-center text-sm font-medium text-white shadow-xl">{notice}</output>}
        ...
        <section className="min-w-0 pb-24 lg:pb-8">
          <header className="sticky top-0 z-30 flex min-h-16 items-end justify-between border-b border-black/5 bg-white/85 px-4 pb-3 pt-[max(12px,env(safe-area-inset-top))] backdrop-blur-xl md:px-8">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button variant="outline" size="icon" aria-label="Tìm kiếm giao dịch" onClick={() => setView('transactions')}><Search /></Button>
              ...
              <Button data-testid="add-transaction" onClick={() => {setEditingTransaction(null);setTransactionPreset(null);setModal('transaction');}} className="hidden bg-[#17231f] text-white sm:flex"><Plus /> Thêm giao dịch</Button>
            </div>
          </header>
          ...
          <div className="mx-auto max-w-[1120px] px-4 py-5 md:px-8 md:py-7">
            ...
            {view === 'overview' && <Overview transactions={transactions} budgets={budgets} accounts={accounts} pending={pending} onView={setView} />}
            {view === 'transactions' && <TransactionsView transactions={transactions} onEdit={(transaction)=>{setEditingTransaction(transaction);setTransactionPreset(null);setModal('transaction');}} onDelete={(transaction)=>{if(!window.confirm(`Xoá giao dịch “${transaction.name}”? Số dư liên quan sẽ được hoàn tác.`)) return;applyTransactionEffects(transaction,-1);setTransactions((items)=>items.filter((item)=>item.id!==transaction.id));toast('Đã xoá giao dịch và hoàn tác số dư liên quan');}} />}
            ...
            {view === 'budgets' && <BudgetsView budgets={budgets} transactions={transactions} onAdd={() => setModal('budget')} onChangeLimit={(id, limit) => setBudgets((items) => items.map((item) => item.id === id ? { ...item, limit:Math.max(0,limit) } : item))} onDelete={(id)=>setBudgets((items)=>items.filter((item)=>item.id!==id))} />}
            {view === 'debts' && <DebtsView debts={debts} onAdd={() => setModal('debt')} onRecord={(debt) => { const action:DebtAction=debt.type==='owe'?'repay':'collect';setEditingTransaction(null);setTransactionPreset({id:crypto.randomUUID(),icon:'🤝',name:'',category:action==='repay'?'Trả nợ':'Thu nợ',time:'Vừa xong',occurredAt:new Date().toISOString(),amount:cashSignFor('debt',action)*Math.max(1,debt.total-debt.paid),group:'debt',reporting:'none',accountId:accounts[0]?.id,debtAction:action,debtId:debt.id,person:debt.person});setModal('transaction'); }} />}
            {view === 'cards' && <CardsView balance={cardBalance} limit={cardLimit} statementDay={cardStatementDay} dueDay={cardDueDay} accounts={accounts} onLimit={(value)=>setCardLimit(Math.max(0,value))} onStatementDay={setCardStatementDay} onDueDay={setCardDueDay} onSettle={(paymentAccountId) => { const paymentAccount=accounts.find((account)=>account.id===paymentAccountId); if(!paymentAccount || cardBalance<=0) return; const paid=cardBalance; const transaction:Transaction={id:crypto.randomUUID(),icon:'💳',name:'Tất toán TPBank EVO Visa',category:'Thanh toán thẻ tín dụng',time:'Vừa xong',occurredAt:new Date().toISOString(),amount:-paid,cardDelta:-paid,systemType:'credit-card-payment',group:'debt',reporting:'none',accountId:paymentAccount.id,accountName:paymentAccount.name,destinationAccountId:CREDIT_CARD_ID,destinationAccountName:CREDIT_CARD_NAME,debtAction:'repay'};setTransactions((items)=>[transaction,...items]);applyTransactionEffects(transaction,1);toast(`Đã tất toán từ ${paymentAccount.name}. Hạn mức đã phục hồi.`); }} />}
      ...
      <MobileNav view={view} pending={pending.length} onView={setView} onAdd={() => {setEditingTransaction(null);setTransactionPreset(null);setModal('transaction');}} />
      {modal && <Modal title={modal === 'transaction' ? (editingTransaction?'Sửa giao dịch':'Thêm giao dịch') : modal === 'budget' ? 'Tạo ngân sách danh mục' : modal === 'account' ? (editingAccount ? 'Sửa tài khoản' : 'Thêm tài khoản') : 'Thêm khoản nợ / cho vay'} onClose={() => {setModal(null);setEditingTransaction(null);setTransactionPreset(null);}}>
        {modal === 'transaction' && <TransactionForm debts={debts} accounts={accounts} initial={editingTransaction ?? transactionPreset} isEditing={Boolean(editingTransaction)} onSave={(transaction) => {
          ...
          setModal(null); setEditingTransaction(null); setTransactionPreset(null); toast(editingTransaction ? 'Đã sửa giao dịch và cân đối lại số dư' : transaction.reporting === 'none' ? 'Đã ghi nhận biến động tiền và công nợ' : 'Đã thêm giao dịch mới');
        }} />}
        {modal === 'budget' && <BudgetForm onSave={(budget) => { if(budgets.some((item)=>item.name===budget.name&&(item.monthKey??monthKey())===budget.monthKey)){toast('Danh mục này đã có ngân sách trong tháng');return;} setBudgets((items) => [...items, budget]); setModal(null); toast('Đã tạo ngân sách danh mục'); }} />}
...
function Overview({ transactions, budgets, accounts, pending, onView }: { transactions: Transaction[]; budgets: Budget[]; accounts: FinancialAccount[]; pending: PendingImport[]; onView: (view: View) => void }) {
  const now = new Date();
  const monthTransactions=transactions.filter((transaction)=>transactionMonth(transaction)===monthKey(now));
  const expense = Math.abs(monthTransactions.filter((t) => t.reporting === 'expense').reduce((sum, item) => sum + item.amount, 0));
  const income = monthTransactions.filter((t) => t.reporting === 'income').reduce((sum, item) => sum + item.amount, 0);
    ...
    <div className="grid gap-5 xl:grid-cols-[1.25fr_.85fr]">
      <div className="space-y-5">
        <section className="surface"><SectionTitle title="Giao dịch chờ duyệt" subtitle="Phải chọn danh mục trước khi ghi nhận" action={<Button variant="ghost" size="sm" onClick={() => onView('pending')}>Xem tất cả</Button>} />
          {pending[0] ? <PendingCard item={pending[0]} compact onCategory={() => onView('pending')} onApprove={() => onView('pending')} onIgnore={() => undefined} /> : <Empty text="Bạn đã xử lý hết email giao dịch." />}
        </section>
...
function PendingCard({ item, accounts = [], compact, onGroup, onCategory, onAccount, onApprove, onIgnore }: { item: PendingImport; accounts?: FinancialAccount[]; compact?: boolean; onGroup?: (value: 'expense' | 'income') => void; onCategory: (value: string) => void; onAccount?: (value: string) => void; onApprove: () => void; onIgnore: () => void }) {
  ...
  return <article className="rounded-2xl border border-[#eadfca] bg-[#fffbf2] p-4" data-testid={`pending-${item.id}`}><div className="flex gap-3"><div className="grid size-10 shrink-0 place-items-center rounded-xl bg-[#7d4cff] text-xs font-black text-white">{item.bank.slice(0, 2).toUpperCase()}</div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{item.merchant}</p><p className="mt-.5 text-xs text-[#7b8982]">{item.bank} · {item.time}</p></div><p className="shrink-0 font-bold">{money(item.amount)}</p></div>{!compact && <div className="mt-4 grid gap-3 sm:grid-cols-3"><label className="block text-xs font-semibold text-[#5f6e67]">Nguồn tiền<select aria-label={`Nguồn tiền ${item.merchant}`} value={item.accountId ?? ''} onChange={(e) => onAccount?.(e.target.value)} className="mt-1.5 h-10 w-full rounded-xl border border-[#d9dfdc] bg-white px-3 text-sm"><option value="">Chọn tài khoản</option><option value={CREDIT_CARD_ID}>{CREDIT_CARD_NAME} · Thẻ tín dụng</option>{accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label><label className="block text-xs font-semibold text-[#5f6e67]">Loại giao dịch<select aria-label={`Loại ${item.merchant}`} value={item.group} onChange={(e) => onGroup?.(e.target.value as 'expense' | 'income')} className="mt-1.5 h-10 w-full rounded-xl border border-[#d9dfdc] bg-white px-3 text-sm"><option value="expense">Khoản Chi</option><option value="income">Khoản Thu</option></select></label><label className="block text-xs font-semibold text-[#5f6e67]">Danh mục<select aria-label={`Danh mục ${item.merchant}`} value={item.category} onChange={(e) => onCategory(e.target.value)} className="mt-1.5 h-10 w-full rounded-xl border border-[#d9dfdc] bg-white px-3 text-sm"><option value="">Chọn danh mục</option>{options.map((category) => <option key={category.value} value={category.value}>{category.label}</option>)}</select></label></div>}<div className="mt-4 flex flex-wrap gap-2">{compact ? <Button size="sm" onClick={onApprove}>Phân loại & duyệt</Button> : <><Button size="sm" onClick={onApprove}>Phê duyệt</Button><Button size="sm" variant="ghost" onClick={onIgnore}>Bỏ qua</Button></>}</div></div></div></article>;
}

function BudgetsView({ budgets, transactions, onAdd, onChangeLimit, onDelete }: { budgets: Budget[]; transactions: Transaction[]; onAdd: () => void; onChangeLimit: (id: string, limit: number) => void; onDelete:(id:string)=>void }) {
  const now = new Date();
  const active=budgets.filter((budget)=>!budget.monthKey||budget.monthKey===monthKey(now));
  return <div className="space-y-5"><div className="flex items-center justify-between"><div><p className="text-sm text-[#708078]">Tháng {now.getMonth()+1}/{now.getFullYear()}</p><p className="text-2xl font-bold">{money(active.reduce((sum, b) => sum + b.limit, 0))}</p></div><Button onClick={onAdd}><Plus /> Thêm danh mục</Button></div><div className="grid gap-4 md:grid-cols-2">{active.map((budget) => <article key={budget.id} className="surface"><div className="flex items-start gap-2"><div className="min-w-0 flex-1"><BudgetRow budget={budget} spent={budgetSpent(budget, transactions)} /></div><Button variant="ghost" size="icon" aria-label={`Xoá ngân sách ${budget.name}`} className="text-[#d43c32]" onClick={()=>{if(window.confirm(`Xoá ngân sách “${budget.name}” của tháng này?`))onDelete(budget.id);}}><Trash2/></Button></div><label className="mt-5 block text-xs font-semibold text-[#5f6e67]">Hạn mức danh mục<input aria-label={`Hạn mức ${budget.name}`} type="number" min="0" value={budget.limit} onChange={(e) => onChangeLimit(budget.id, Number(e.target.value))} className="field mt-1.5" /></label></article>)}</div>{!active.length&&<Empty text="Chưa có ngân sách cho tháng này."/>}</div>;
}
...
function DebtsView({ debts, onAdd, onRecord }: { debts: Debt[]; onAdd: () => void; onRecord: (debt:Debt) => void }) {
  ...
  return <div className="space-y-5"><div className="grid gap-4 sm:grid-cols-2"><SummaryCard icon={<ArrowUpRight />} label="Bạn còn phải trả" value={money(owe)} tone="red" /><SummaryCard icon={<ArrowDownLeft />} label="Người khác còn nợ bạn" value={money(lend)} tone="green" /></div><div className="flex justify-end"><Button onClick={onAdd}><Plus /> Thêm khoản mới</Button></div><div className="grid gap-4 md:grid-cols-2">{debts.map((debt) => { const remain = debt.total - debt.paid; const ratio = debt.total>0?Math.round(debt.paid / debt.total * 100):0; return <article key={debt.id} className="surface"><div className="flex items-start justify-between"><div><Badge variant="secondary" className={debt.type === 'owe' ? 'bg-[#fff0ed] text-[#a94135]' : 'bg-[#eaf7ee] text-[#257545]'}>{debt.type === 'owe' ? 'Bạn đang nợ' : 'Bạn cho vay'}</Badge><h3 className="mt-3 text-lg font-bold">{debt.person}</h3><p className="text-xs text-[#7b8982]">Hạn trả: {debt.due}</p></div><HandCoins className="size-6 text-[#7b8982]" /></div><div className="mt-5 flex justify-between text-sm"><span>Còn lại</span><strong>{money(remain)}</strong></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-[#edf1ef]"><div className="h-full rounded-full bg-[#7b55c7]" style={{ width: `${ratio}%` }} /></div><p className="mt-2 text-xs text-[#7b8982]">Đã thanh toán {money(debt.paid)} / {money(debt.total)}</p>{remain > 0 ? <Button className="mt-4 w-full" variant="outline" onClick={() => onRecord(debt)}>{debt.type==='owe'?'Ghi nhận trả nợ':'Ghi nhận thu nợ'}</Button> : <div className="mt-4 rounded-xl bg-[#eaf7ee] p-2.5 text-center text-sm font-semibold text-[#257545]">Đã hoàn tất</div>}</article>; })}</div></div>;
}

function CardsView({ balance, limit, statementDay, dueDay, accounts, onLimit, onStatementDay, onDueDay, onSettle }: { balance: number; limit: number; statementDay:number; dueDay:number; accounts: FinancialAccount[]; onLimit: (value: number) => void; onStatementDay:(value:number)=>void; onDueDay:(value:number)=>void; onSettle: (paymentAccountId: string) => void }) {
  ...
  return <div className="grid gap-5 lg:grid-cols-[1.1fr_.9fr]"><section className="rounded-[26px] bg-gradient-to-br from-[#623cc5] to-[#332165] p-6 text-white shadow-xl"><div className="flex justify-between"><div><p className="text-xs uppercase tracking-[.15em] text-white/60">TPBank EVO Visa</p><p className="mt-7 text-2xl font-bold">•••• 1962</p></div><CreditCard className="size-8 text-white/70" /></div><div className="mt-10 grid grid-cols-2 gap-4"><div><p className="text-xs text-white/60">Dư nợ hiện tại</p><p className="font-bold">{money(balance)}</p></div><div><p className="text-xs text-white/60">Hạn mức khả dụng</p><p className="font-bold">{money(available)}</p></div></div></section><section className="surface"><h2 className="font-bold">Chu kỳ thanh toán</h2><div className="mt-4 grid grid-cols-2 gap-3"><label className="rounded-xl bg-[#f3f6f4] p-3 text-[11px] text-[#7b8982]">Ngày chốt sao kê<input aria-label="Ngày chốt sao kê" type="number" min="1" max="31" value={statementDay} onChange={(event)=>onStatementDay(Math.min(31,Math.max(1,Number(event.target.value))))} className="mt-1 w-full bg-transparent text-sm font-semibold text-[#17231f] outline-none"/></label><label className="rounded-xl bg-[#f3f6f4] p-3 text-[11px] text-[#7b8982]">Ngày đáo hạn<input aria-label="Ngày đáo hạn" type="number" min="1" max="31" value={dueDay} onChange={(event)=>onDueDay(Math.min(31,Math.max(1,Number(event.target.value))))} className="mt-1 w-full bg-transparent text-sm font-semibold text-[#17231f] outline-none"/></label></div><div className="mt-5 h-2 overflow-hidden rounded-full bg-[#edf1ef]"><div className="h-full bg-[#7b55c7]" style={{ width: `${ratio}%` }} /></div><p className="mt-2 text-xs text-[#7b8982]">Đã dùng {ratio}% hạn mức</p><label className="mt-5 block text-xs font-semibold">Hạn mức thẻ<input aria-label="Hạn mức thẻ tín dụng" className="field mt-1.5" type="number" min="0" value={limit} onChange={(e) => onLimit(Number(e.target.value))} /></label><label className="mt-4 block text-xs font-semibold">Tất toán từ tài khoản<select aria-label="Tài khoản tất toán thẻ" className="field mt-1.5" value={paymentAccountId} onChange={(event)=>setPaymentAccountId(event.target.value)}><option value="">Chọn nguồn trả thẻ</option>{accounts.map((account)=><option key={account.id} value={account.id}>{account.name} · {money(account.balance)}</option>)}</select></label><Button className="mt-4 w-full" disabled={balance === 0 || !paymentAccountId} onClick={()=>onSettle(paymentAccountId)}>Tất toán dư nợ</Button><p className="mt-2 text-center text-[11px] text-[#7b8982]">Số tiền trả thẻ sẽ được trừ khỏi tài khoản đã chọn và phục hồi hạn mức.</p></section></div>;
}
...
function TransactionsView({ transactions, onEdit, onDelete }: { transactions: Transaction[]; onEdit: (transaction:Transaction)=>void; onDelete:(transaction:Transaction)=>void }) { const [query,setQuery]=useState('');const normalized=query.trim().toLocaleLowerCase('vi');const filtered=normalized?transactions.filter((item)=>`${item.name} ${item.category} ${item.accountName??''} ${item.destinationAccountName??''}`.toLocaleLowerCase('vi').includes(normalized)):transactions;return <section className="surface"><SectionTitle title={`${filtered.length} giao dịch`} subtitle="Dữ liệu đã được ghi nhận" /><div className="relative mb-3"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-[#8e8e93]"/><input aria-label="Tìm giao dịch" className="field pl-10" placeholder="Tìm nội dung, danh mục hoặc tài khoản" value={query} onChange={(event)=>setQuery(event.target.value)}/></div>{filtered.length?<TransactionList transactions={filtered} onEdit={onEdit} onDelete={onDelete}/>:<Empty text="Không tìm thấy giao dịch phù hợp."/>}</section>; }
function TransactionList({ transactions, onEdit, onDelete }: { transactions: Transaction[]; onEdit?: (transaction:Transaction)=>void; onDelete?: (transaction:Transaction)=>void }) { return <div className="divide-y divide-[#edf1ef]">{transactions.map((t) => <div key={t.id} className="flex flex-wrap items-center gap-3 py-3.5 sm:flex-nowrap"><div className="grid size-10 shrink-0 place-items-center rounded-xl bg-[#f1f4f2] text-lg">{t.icon}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{t.name}</p><p className="line-clamp-2 text-xs text-[#7b8982]">{t.category} · {t.accountName || 'Chưa xác định nguồn'}{t.destinationAccountName ? ` ↔ ${t.destinationAccountName}` : ''} · {t.time}</p>{t.reporting === 'none' && <p className="mt-0.5 text-[10px] font-semibold text-[#7b55c7]">Biến động tài sản/công nợ · không tính thu chi</p>}</div><p className={`shrink-0 text-sm font-bold ${t.amount > 0 ? 'text-[#248a4b]' : ''}`}>{t.amount > 0 ? '+ ' : '- '}{money(Math.abs(t.amount))}</p>{onEdit&&onDelete&&<div className="ml-auto flex shrink-0 gap-1 sm:ml-0">{!t.systemType&&<Button variant="ghost" size="icon" aria-label={`Sửa ${t.name}`} onClick={()=>onEdit(t)}><Pencil /></Button>}<Button variant="ghost" size="icon" aria-label={`Xoá ${t.name}`} className="text-[#d43c32]" onClick={()=>onDelete(t)}><Trash2 /></Button></div>}</div>)}</div>; }

function Modal({ title, children, onClose }: { title: string; children: React.ReactNode; onClose: () => void }) { return <dialog open aria-label={title} className="fixed inset-0 z-[70] m-0 grid size-full max-h-none max-w-none place-items-end border-0 bg-black/35 p-0 backdrop-blur-sm sm:place-items-center sm:p-4"><section className="max-h-[94dvh] w-full overflow-auto rounded-t-[32px] bg-white px-5 pb-[max(24px,env(safe-area-inset-bottom))] pt-4 shadow-2xl sm:max-w-lg sm:rounded-[28px] sm:p-6"><div className="mx-auto mb-3 h-1.5 w-10 rounded-full bg-[#d1d1d6] sm:hidden"/><div className="mb-5 flex items-center justify-between"><h2 className="text-xl font-bold tracking-[-.02em]">{title}</h2><Button variant="ghost" size="icon" aria-label="Đóng" onClick={onClose}><X /></Button></div>{children}</section></dialog>; }
function TransactionForm({ debts, accounts, initial, isEditing, onSave }: { debts: Debt[]; accounts: FinancialAccount[]; initial: Transaction | null; isEditing:boolean; onSave: (item: Transaction) => void }) {
  ...
  const debtMax = activeDebt ? activeDebt.total-activeDebt.paid+(isEditing&&initial?.debtId===activeDebt.id&&['repay','collect'].includes(initial.debtAction??'')?Math.abs(initial.amount):0) : undefined;
  ...
  return <form onSubmit={(e) => {
    ...
    <Field label="Số tiền"><input className="field" type="number" min="1" max={debtMax} inputMode="numeric" placeholder="0" value={amount} onChange={(e) => setAmount(e.target.value)} required /></Field>
    ...
    <Button type="submit" className="h-12 w-full">{isEditing ? 'Lưu thay đổi' : 'Lưu giao dịch'}</Button>
  </form>;
}
...
function BudgetForm({ onSave }: { onSave: (item: Budget) => void }) { const [name, setName] = useState(expenseCategories[0].label); const [limit, setLimit] = useState(''); return <form onSubmit={(e) => { e.preventDefault(); if (!name || !limit || Number(limit)<=0) return; onSave({ id: crypto.randomUUID(), name, limit: Number(limit), spent: 0, icon: '🎯', color: '#2c8a54', monthKey: monthKey() }); }} className="space-y-4"><Field label="Danh mục chi tiêu"><select aria-label="Danh mục ngân sách" className="field" value={name} onChange={(event)=>setName(event.target.value)}>{expenseCategories.map((category)=><option key={category.label} value={category.label}>{category.label}</option>)}</select><p className="mt-2 text-xs font-normal text-[#7b8982]">Ngân sách nhóm sẽ tự tính cả các danh mục con, ví dụ Hóa đơn & tiện ích.</p></Field><Field label="Ngân sách tháng này"><input className="field" type="number" min="1" inputMode="numeric" placeholder="0" value={limit} onChange={(e) => setLimit(e.target.value)} required /></Field><Button type="submit" className="h-11 w-full">Tạo ngân sách</Button></form>; }
