    ...
    <main className="min-h-dvh bg-[#f5f7f6] text-[#17231f]">
      ...
      {modal && <Modal title={modal === 'transaction' ? 'Thêm giao dịch' : modal === 'budget' ? 'Tạo ngân sách danh mục' : modal === 'account' ? (editingAccount ? 'Sửa tài khoản' : 'Thêm tài khoản') : 'Thêm khoản nợ / cho vay'} onClose={() => setModal(null)}>
        {modal === 'transaction' && <TransactionForm debts={debts} accounts={accounts} onSave={(transaction) => {
          ...
          if (transaction.accountId === CREDIT_CARD_ID) setCardBalance((balance) => Math.max(0, balance - transaction.amount));
          else if (transaction.accountId) setAccounts((items) => items.map((account) => account.id === transaction.accountId ? { ...account, balance: account.balance + transaction.amount } : account.id === transaction.destinationAccountId ? { ...account, balance: account.balance - transaction.amount } : account));
...
function approvePending(id: string, pending: PendingImport[], accounts: FinancialAccount[], setPending: React.Dispatch<React.SetStateAction<PendingImport[]>>, setTransactions: React.Dispatch<React.SetStateAction<Transaction[]>>, setAccounts: React.Dispatch<React.SetStateAction<FinancialAccount[]>>, setCardBalance: React.Dispatch<React.SetStateAction<number>>, toast: (message: string) => void) {
  ...
  if (item.accountId === CREDIT_CARD_ID) {
    setTransactions((items) => [{ id: crypto.randomUUID(), icon: '💳', name: item.merchant, category: item.category, time: item.time, amount: signedAmount, group: item.group, reporting: reportingBucketFor(item.group, item.category), accountId: CREDIT_CARD_ID, accountName: CREDIT_CARD_NAME }, ...items]);
